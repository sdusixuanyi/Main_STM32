...
const float Control_Unit[18][20]=
{
  ...
  //好盈乐天20A默认固件F330
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,30  ,0  ,0 , 80,   2.80   ,0.0000  ,0.00  ,0  ,0 , 300,  1 ,  1 ,  1 },//Pitch_Angle;偏航角度
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,500 ,0  ,0 , 300,  0.82   ,6.5000  ,2.50  ,0  ,0 , 500,  1 ,  1 ,  1 },//Pitch_Gyro;偏航角速度
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,30  ,0  ,0 , 80,   2.80   ,0.0000  ,0.00  ,0  ,0 , 300,  1 ,  1 ,  1 },//Roll_Angle;横滚角
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,500 ,0  ,0 , 300,  0.82   ,6.5000  ,2.50  ,0  ,0 , 500,  1 ,  1 ,  1 },//Roll_Gyro;横滚角速度
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,45  ,0  ,0 , 150 , 5.00   ,0.0000  ,0.00  ,0  ,0 , 300,  1 ,  1 ,  1 },//Yaw_Angle;偏航角
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,250 ,0  ,0 , 100,  2.00   ,0.5000  ,0.00  ,0  ,0 , 300,  1 ,  1 ,  1 },//Yaw_Gyro;偏航角速度   
  //定高参数
  ...
  */
  ...
  /*************加速度控制器****************/
  //期望最大加速度500cm/s^2
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,700  ,0  ,0 ,500,0.12    ,1.5000    ,0.0  ,0   ,0 ,700,  1 ,  1 ,  1 },//垂直加速度控制器
  ...
  /*************光流位置、速度控制器****************/
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,100  ,15  ,0 ,15,  0.25   ,0.00    ,0    ,0   ,0 ,30,   1 ,  1 ,  1 },//水平经度方向加速度控制器
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,30   ,15  ,0 ,200, 4.50   ,0.10    ,0.0  ,0   ,0 ,450 , 1 ,  1 ,  1 },//水平维度方向加速度控制器
  {1  ,1 ,0 ,0 ,0 ,0 , 0 ,30   ,30  ,0 ,10,  0.20   ,0     ,4.0  ,0    ,0 ,500  , 1 ,  1 ,  1 },//水平维度方向加速度控制器
};
...
void Total_PID_Init(void)
{
  ...
  Set_Cutoff_Frequency(Imu_Sampling_Freq, 5 ,&Control_Device_Err_LPF_Parameter);
  Set_Cutoff_Frequency(Imu_Sampling_Freq, 20,&Control_Device_Div_LPF_Parameter);
  Set_Cutoff_Frequency(Imu_Sampling_Freq, 30,&Control_Device_Div_LPF_Parameter_For_Gyro);
}
...
float PID_Control_Div_LPF_For_Gyro(PID_Controler *Controler)
{
  ...
  Controler->Dis_Error_History[0]=constrain_float(Controler->Dis_Error_History[0],-250,250);//微分项限幅
  Controler->Adaptable_Kd=Controler->Kd*(1+Controler->Dis_Error_History[0]/250.0f);//自适应微分参数
