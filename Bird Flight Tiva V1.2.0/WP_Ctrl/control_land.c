...
void landon_earth_check(void)//自检触地进入怠速模式
{
  ...
  if(Throttle_Output<=Minimal_Thrust_Threshold
     &&Gyro_Length<=20.0f//触地后无旋转，合角速度小于20deg/s
       &&ABS(NamelessQuad.Speed[_YAW])<=20.0f//惯导竖直轴速度+-20cm/s
         &&Unwanted_Lock_Flag==0
           &&pos_handmove_check()==FALSE)//无水平手动操作
    Landon_Earth_Cnt++;
  else Landon_Earth_Cnt/=2;
  
  if(Landon_Earth_Cnt>=5000)  Landon_Earth_Cnt=5000;//防止溢出
  if(Landon_Earth_Cnt>=200*3.0)//持续3.0S
  {
...
void land_nogps_run()
{
  if(Landon_Earth_Flag==0)//未触地，有姿态控制参与
  {
    ...
    if(NamelessQuad.Position[_YAW]<=Nav_Safety_Height)//相对初始高度10M以内,暂时忽略漂移
...
void land_gps_run()
{
  ...
  if(Landon_Earth_Flag==0)//未触地，有姿态控制参与
  {
    if(check_pos_handmove==FALSE)//无水平打杆动作位
    {
      ...
      {
        if(NamelessQuad.Position[_YAW]<Nav_Safety_Height)//当切返航瞬间的高度小于安全高度时，保持当前水平位置，攀升至安全高度再返航
        {
      ...
      {
        if(NamelessQuad.Position[_YAW]>=Nav_Safety_Height//当返航的高度超过安全高度时，保持当前高度，飞至home正上方即可
